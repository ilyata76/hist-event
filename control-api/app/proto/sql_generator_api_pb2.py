# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: sql-generator-api.proto
"""Generated protocol buffer code."""
from google.protobuf import descriptor as _descriptor
from google.protobuf import descriptor_pool as _descriptor_pool
from google.protobuf import symbol_database as _symbol_database
from google.protobuf.internal import builder as _builder
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(b'\n\x17sql-generator-api.proto\x12\x11sql_generator_api\"\x07\n\x05PingR\"\x15\n\x05PongR\x12\x0c\n\x04pong\x18\x01 \x01(\t\"\x85\x01\n\nManyFilesR\x12&\n\x05\x66iles\x18\x01 \x03(\x0b\x32\x17.sql_generator_api.File\x12.\n\x08main_sql\x18\x02 \x01(\x0b\x32\x17.sql_generator_api.FileH\x00\x88\x01\x01\x12\x12\n\nidentifier\x18\x03 \x01(\tB\x0b\n\t_main_sql\",\n\x06Status\x12\x0e\n\x06status\x18\x01 \x01(\t\x12\x12\n\nidentifier\x18\x03 \x01(\t\"H\n\x04\x46ile\x12\x0f\n\x07storage\x18\x01 \x01(\t\x12\x0c\n\x04path\x18\x02 \x01(\t\x12\x10\n\x08\x66ilename\x18\x03 \x01(\t\x12\x0f\n\x07keyword\x18\x04 \x01(\t2\xa4\x02\n\x0fSQLGeneratorAPI\x12<\n\x04Ping\x12\x18.sql_generator_api.PingR\x1a\x18.sql_generator_api.PongR\"\x00\x12\x46\n\x08Validate\x12\x1d.sql_generator_api.ManyFilesR\x1a\x19.sql_generator_api.Status\"\x00\x12\x43\n\x05Parse\x12\x1d.sql_generator_api.ManyFilesR\x1a\x19.sql_generator_api.Status\"\x00\x12\x46\n\x08Generate\x12\x1d.sql_generator_api.ManyFilesR\x1a\x19.sql_generator_api.Status\"\x00\x62\x06proto3')

_globals = globals()
_builder.BuildMessageAndEnumDescriptors(DESCRIPTOR, _globals)
_builder.BuildTopDescriptorsAndMessages(DESCRIPTOR, 'sql_generator_api_pb2', _globals)
if _descriptor._USE_C_DESCRIPTORS == False:
  DESCRIPTOR._options = None
  _globals['_PINGR']._serialized_start=46
  _globals['_PINGR']._serialized_end=53
  _globals['_PONGR']._serialized_start=55
  _globals['_PONGR']._serialized_end=76
  _globals['_MANYFILESR']._serialized_start=79
  _globals['_MANYFILESR']._serialized_end=212
  _globals['_STATUS']._serialized_start=214
  _globals['_STATUS']._serialized_end=258
  _globals['_FILE']._serialized_start=260
  _globals['_FILE']._serialized_end=332
  _globals['_SQLGENERATORAPI']._serialized_start=335
  _globals['_SQLGENERATORAPI']._serialized_end=627
# @@protoc_insertion_point(module_scope)
